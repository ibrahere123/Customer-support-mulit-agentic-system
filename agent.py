...
class SupportAgent:
    def __init__(self):
        ...
        self.max_context_length = 4000  # Increased context window
        self.context_summary_threshold = 3000  # Summarize when context reaches this size
        self.max_history_length = 10  # Max conversation turns to keep in context
    ...
    def should_escalate(self, confidence: float, user_id: str) -> bool:
        ...
        # Count medium-low confidence responses
        if confidence < self.escalation_threshold:
            self.escalation_count[user_id] += 1
        
        # Reset counter on high confidence responses
        if confidence > 0.7:
            self.escalation_count[user_id] = 0
        
        print(f"🚨 Escalation count: {self.escalation_count[user_id]}/2")
        return self.escalation_count[user_id] >= 2
    ...
    def remember_context(self, user_id: str, query: str, response: str):
        """Robust context management with proper initialization"""
        try:
            ...
            self.user_contexts[user_id].append({"role": "user", "content": query})
            self.user_contexts[user_id].append({"role": "assistant", "content": response})
            
            # Keep only recent history to manage context length
            if len(self.user_contexts[user_id]) > self.max_history_length * 2:  # *2 because each exchange has 2 messages
                # Keep the last max_history_length exchanges
